...
const paymentCallback = async (query: Record<string, any>) => {
  const transactionResult = await prisma.$transaction(
    async (tx) => {
      ...
      if (!paymentID) {
        throw new AppError(httpstatus.NOT_FOUND, "Payment ID is missing");
      }

      if (!status) {
        throw new AppError(httpstatus.NOT_FOUND, "status missing");
      }

      const bkashIdToken = await getBkashIdToken();

      if (!bkashIdToken) {
        throw new AppError(httpstatus.NOT_FOUND, "Bkash id not found");
      }
